...
class ImmdValGen_IO[T <: Data](uInt: T, sInt: T) extends Bundle {
	val instr = Input(uInt)
	val immd_se = Output(sInt)
	val branch_se = Output(sInt)
	val j_sel = Output(sInt)
    val pc=Input(sInt)
}
class ImmediateGen(sizeUint:UInt,sizeSint:SInt) extends Module {
	val io = IO(new ImmdValGen_IO(sizeUint:UInt, sizeSint:SInt))
	io.immd_se := 0.S
	io.branch_se:=0.S
	io.j_sel:=0.S
    ...
    .elsewhen(io.instr(6,0)==="b1100011".U){           //SB_Immediate
		io.branch_se := (Cat(Fill(19,io.instr(31)),io.instr(31),io.instr(7),io.instr(30,25),io.instr(11,8), 0.U)+io.pc.asUInt).asSInt
	}    
    .elsewhen(io.instr(6,0)==="b1101111".U){      //UJ_Immediate
		io.j_sel:= (Cat(Fill(11,io.instr(31)),io.instr(31),io.instr(19,12),io.instr(20),io.instr(30,25),io.instr(24,21), 0.U)).asSInt
